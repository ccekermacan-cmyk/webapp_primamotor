...
interface UserData {
  ...
  link_image: string;
  tokenkey: string;
  user_level: number; // Tambahkan ini
}
...
export default function AkunPage() {
  ...
  // Efek Warna Gradasi Background Dinamis
  useEffect(() => {
    ...
    const img = new Image();
    img.crossOrigin = "Anonymous"; // Penting agar bisa membaca data pixel gambar dari luar domain
    ...
    img.onload = () => {
      ...
      // Resize kecil agar proses komputasi ringan (tetap akurat untuk rata-rata warna)
      canvas.width = 50;
      canvas.height = 50;
      ...
      try {
        ...
        // Buat warna gradasi dari warna dominan (terang ke agak gelap)
  ...
  const fetchProfile = async () => {
      try {
        ...
        if (userId) {
          ...
          // SINKRONISASI AMAN: Hanya set jika datanya ada
  ...
  const fetchGaji = async () => {
    try {
      // PERBAIKAN: Tambahkan $autoCancel: false
      const result = await pb.collection('gaji').getList<Gaji>(page, perPage, {
        filter: `person = "${userData?.username}"`,
        sort: '-created_at',
        expand: 'ref',
        $autoCancel: false 
      });
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      // Pastikan payload bersih dan tidak mengandung 'undefined'
      const payload = {
        name: formData.name,
        email: formData.email,
        link_image: formData.link_image,
        tokenkey: formData.tokenkey,
        // Tambahkan password jika sedang diubah
      ...
      // Setelah update berhasil, panggil fetch ulang untuk mengambil data terbaru
      await fetchProfile(); 
    ...
    <div className="max-w-4xl mx-auto p-8 space-y-8">
      ...
      <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-white overflow-hidden relative group">
        ...
        <div className="px-8 pb-8 flex flex-col items-center -mt-16 text-center">
          <div className="relative">
            ...
            />
            <div className="absolute bottom-0 right-0 p-2 bg-blue-600 text-white rounded-xl border-2 border-white shadow-lg">
              <Camera size={16} />
            </div>
          </div>
          
          <h2 className="mt-4 text-3xl font-black text-slate-800 tracking-tight">{userData?.name || userData?.username}</h2>
          <div className="flex gap-2 mt-1">
            <span className="px-3 py-1 bg-blue-50 text-blue-600 text-xs font-black uppercase rounded-lg border border-blue-100 flex items-center gap-1">
              <Shield size={12} /> {userData?.status || 'User'}
            </span>
            <span className="px-3 py-1 bg-slate-50 text-slate-500 text-xs font-black uppercase rounded-lg border border-slate-100 flex items-center gap-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full mt-8">
            ...
            <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100 text-left">
              <div className="p-2 bg-white rounded-xl shadow-sm text-indigo-500"><Lock size={20} /></div>
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase">Token Key</p>
                <p className="font-bold text-slate-700 truncate max-w-[150px]">{userData?.tokenkey || 'Not Set'}</p>
              </div>
            </div>
          </div>
          ...
          <button 
            ...
            className="mt-8 flex items-center gap-2 px-8 py-3 bg-slate-800 text-white rounded-2xl font-black hover:bg-slate-900 transition-all shadow-xl shadow-slate-200"
          >
      ...
      <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 p-8">
        ...
        <div className="space-y-3">
          {salaryList.length === 0 ? (
            <div className="text-center py-10 text-slate-400 font-bold">Belum ada riwayat gaji tercatat.</div>
          ) : (
            salaryList.map((item) => {
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-slate-700">Slip Gaji Bulanan</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="text-right mr-4">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Diterima</p>
                      <p className="font-black text-emerald-600 text-lg">******</p>
                    </div>
                    <div className="p-2 bg-slate-200/50 text-slate-400 rounded-full group-hover:bg-emerald-500 group-hover:text-white transition-all">
                      <Eye size={18} />
                    </div>
                  </div>
                </div>
        ...
        <div className="flex justify-between items-center mt-8 pt-6 border-t border-slate-100">
          ...
          <span className="text-sm font-black text-slate-400 uppercase">Hal {page} / {totalPages}</span>
          <button 
      ...
      <Modal isOpen={modalType === 'editProfile'} onClose={() => setModalType(null)} title="Update Profil">
        <form onSubmit={handleUpdateProfile} className="space-y-6 max-h-[80vh] overflow-y-auto pr-2 custom-scrollbar">
          ...
          <button type="submit" disabled={isProcessing} className="w-full py-4 bg-slate-800 text-white rounded-2xl font-black shadow-xl mt-4">
            {isProcessing ? 'MENYIMPAN...' : 'SIMPAN PERUBAHAN'}
          </button>
        </form>
      </Modal>

      {/* --- MODAL: VERIFIKASI PASSWORD (SECURITY LAYER) --- */}
      <Modal isOpen={modalType === 'verifyPassword'} onClose={() => setModalType(null)} title="Security Check">
        <form onSubmit={checkSalaryPassword} className="text-center space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Masukkan password login Anda untuk melihat rincian gaji.</p>
          </div>
          <input 
            type="password" 
            required
            ...
            className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-center text-xl outline-none focus:ring-4 focus:ring-rose-100 transition-all text-slate-800"
          />
          <div className="flex gap-3">
            <button type="button" onClick={() => setModalType(null)} className="flex-1 py-4 bg-slate-100 rounded-2xl font-black text-slate-400">BATAL</button>
            <button type="submit" disabled={isProcessing} className="flex-1 py-4 bg-slate-800 text-white rounded-2xl font-black">VERIFIKASI</button>
          </div>
        </form>
      </Modal>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3 p-4 bg-slate-50 border border-slate-200 rounded-2xl">
                <div>
                  ...
                  <p className="font-bold text-slate-700 text-xs">{selectedGaji.id}</p>
                </div>
              ...
              <div className="max-h-[50vh] overflow-y-auto pr-2 space-y-4 custom-scrollbar">
                ...
                <div className="border border-blue-100 rounded-2xl overflow-hidden">
                  <div className="bg-blue-50 px-4 py-3 border-b border-blue-100 flex justify-between items-center">
                    ...
                    <span className="font-black text-blue-700">{formatRupiah(total1)}</span>
                  </div>
                  <div className="bg-white divide-y divide-slate-50 text-xs">
                    <div className="flex justify-between p-3"><span className="text-slate-500 font-medium">Gaji Pokok</span><span className="font-bold text-slate-800">{formatRupiah(t1.pokok)}</span></div>
                    <div className="flex justify-between p-3"><span className="text-slate-500 font-medium">Tunjangan</span><span className="font-bold text-slate-800">{formatRupiah(t1.tunjangan)}</span></div>
                    <div className="flex justify-between p-3"><span className="text-slate-500 font-medium">Bonus 1</span><span className="font-bold text-slate-800">{formatRupiah(t1.bonus_1)}</span></div>
                    <div className="flex justify-between p-3"><span className="text-slate-500 font-medium">Bonus 2</span><span className="font-bold text-slate-800">{formatRupiah(t1.bonus_2)}</span></div>
                    <div className="flex justify-between p-3"><span className="text-slate-500 font-medium">Bonus 3</span><span className="font-bold text-slate-800">{formatRupiah(t1.bonus_3)}</span></div>
                    <div className="flex justify-between p-3"><span className="text-slate-500 font-medium">Bonus 4</span><span className="font-bold text-slate-800">{formatRupiah(t1.bonus_4)}</span></div>
                    <div className="flex justify-between p-3"><span className="text-slate-500 font-medium">Program</span><span className="font-bold text-slate-800">{formatRupiah(t1.program)}</span></div>
                    <div className="flex justify-between p-3"><span className="text-slate-500 font-medium">Lembur</span><span className="font-bold text-slate-800">{formatRupiah(t1.lembur)}</span></div>
                  </div>
                </div>

                {/* --- TOTAL 2: POTONGAN KEHADIRAN --- */}
                <div className="border border-rose-100 rounded-2xl overflow-hidden">
                  <div className="bg-rose-50 px-4 py-3 border-b border-rose-100 flex justify-between items-center">
                    ...
                    <span className="font-black text-rose-700">-{formatRupiah(total2)}</span>
                  </div>
                  <div className="bg-white divide-y divide-slate-50 text-xs">
                    <div className="flex justify-between p-3">
                      <span className="text-slate-500 font-medium">Alfa ({selectedGaji.alfa || 0}x)</span>
                      <span className="font-bold text-rose-600">{formatRupiah(t2.alfa)}</span>
                    </div>
                    <div className="flex justify-between p-3">
                      <span className="text-slate-500 font-medium">Setengah Hari ({selectedGaji.setengah_hari || 0}x)</span>
                      <span className="font-bold text-rose-600">{formatRupiah(t2.setengah_hari)}</span>
                    </div>
                    <div className="flex justify-between p-3">
                      <span className="text-slate-500 font-medium">Sakit ({selectedGaji.sakit || 0}x)</span>
                      <span className="font-bold text-rose-600">{formatRupiah(t2.sakit)}</span>
                    </div>
                    <div className="flex justify-between p-3">
                      <span className="text-slate-500 font-medium">Telat ({selectedGaji.telat || 0}x)</span>
                      <span className="font-bold text-rose-600">{formatRupiah(t2.telat)}</span>
                    </div>
                  </div>
                </div>

                {/* --- TOTAL 3: POTONGAN LAINNYA --- */}
                <div className="border border-orange-100 rounded-2xl overflow-hidden">
                  <div className="bg-orange-50 px-4 py-3 border-b border-orange-100 flex justify-between items-center">
                    ...
                    <span className="font-black text-orange-700">-{formatRupiah(total3)}</span>
                  </div>
                  <div className="bg-white divide-y divide-slate-50 text-xs">
                    <div className="flex justify-between p-3"><span className="text-slate-500 font-medium">BPJS</span><span className="font-bold text-orange-600">{formatRupiah(t3.bpjs)}</span></div>
                    <div className="flex justify-between p-3"><span className="text-slate-500 font-medium">Bon Diambil</span><span className="font-bold text-orange-600">{formatRupiah(t3.bon_diambil)}</span></div>
                    <div className="flex justify-between p-3"><span className="text-slate-500 font-medium">Bon Dibayar</span><span className="font-bold text-orange-600">{formatRupiah(t3.bon_dibayar)}</span></div>
                  </div>
                </div>

              </div>

              <button onClick={() => setModalType(null)} className="w-full py-4 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-2xl font-black transition-colors">TUTUP RINCIAN</button>
            </div>
          );
        })()}
      </Modal>

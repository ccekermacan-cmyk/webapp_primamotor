...
export default function PeoplePage() {
  ...
  const perPage = 5; // Dibatasi 5 baris sesuai dengan tampilan gambar referensi

const fetchPeople = async () => {
    setLoading(true);
    try {
      let conditions: string[] = ['kategori ~ "Person"']; 
      
      // Filter Jenis (Sudah benar)
      if (filterJenis !== 'all') {
        conditions.push(`jenis ~ "${filterJenis}"`);
      }

      // REVISI PENTING: Jangan gunakan parameter binding untuk OR multiple columns
      // Gunakan string literal biasa yang dibersihkan (replace quotes) agar tidak error 400
      if (searchTerm.trim() !== '') {
        const cleanTerm = searchTerm.replace(/"/g, ''); // bersihkan tanda kutip ganda jika user mengetiknya
        conditions.push(`(text_1 ~ "${cleanTerm}" || text_2 ~ "${cleanTerm}" || text_3 ~ "${cleanTerm}")`);
      }

      // Menggabungkan filter
      const finalFilter = conditions.join(' && ');
      
      console.log("Mengirim filter ke server:", finalFilter);
      ...
      console.error("Gagal load data person:", e);
      // Agar tidak mengganggu UX saat user mengetik cepat, kita hilangkan alert
  ...
  // Efek memicu kueri ulang jika halaman, tab, atau kata kunci pencarian berubah
  useEffect(() => {
    const delayDebounce = setTimeout(() => {
      fetchPeople();
    }, 500); // Debounce pencarian agar tidak memberatkan server saat mengetik
    
    return () => clearTimeout(delayDebounce);
  }, [page, filterJenis, searchTerm]);

  // --- 3. MUTATION HANDLERS (SAVE & DELETE) ---
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      // Memastikan bahwa data "Jenis" tersimpan dengan rapi (Kapital di awal)
      const formattedJenis = currentPerson.jenis 
        ? currentPerson.jenis.charAt(0).toUpperCase() + currentPerson.jenis.slice(1).toLowerCase()
        : 'Customer';
      ...
      const payload = {
        ...
        kategori: 'Person', // Standarkan ke 'Person' (P Kapital)
      ...
      setCurrentPerson({});
      setPage(1); // Kembalikan ke halaman pertama setelah menyimpan
      fetchPeople();
    } catch (e) {
      console.error(e);
  ...
  // Helper mendapatkan inisial avatar melingkar (seperti UI gambar)
    ...
    <div className="p-8 bg-[#f8fafc] min-h-screen font-sans text-slate-600">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm flex flex-col lg:flex-row items-center justify-between gap-4">
          ...
          {/* Rentang Tab Kiri (All, Customer, Supplier) */}
                  ...
                  people.map(p => {
                    ...
                    return (
                      <tr key={p.id} className="hover:bg-slate-50/50 transition-colors group">
                        ...
                        {/* Kolom 1: Profil (Avatar bulat + Nama + Deskripsi) */}
                        ...
                        {/* Kolom 2: Status Tagging (Cyan / Orange) */}
                        <td className="py-4 px-6 whitespace-nowrap">
                          <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider border ${
                            isCustomer 
                              ? 'bg-cyan-50/60 text-cyan-700 border-cyan-100' 
      ...
      {/* --- MODAL DIALOG: ADD / EDIT DIALOG FORM --- */}

    ...
    <div className="p-8 h-full flex flex-col bg-slate-50 relative">
      ...
      <div className="flex gap-2 mb-6 border-b border-slate-200">
        <button
          ...
          className={`flex-1 px-6 py-3 text-sm font-black uppercase tracking-wider rounded-t-2xl transition-all ${
            activeTab === 'accounts'
      ...
      <Modal isOpen={modalType === 'form'} onClose={handleCloseModal} title={isEditMode ? "Edit Transaksi" : "Catat Transaksi"}>
        <form onSubmit={submitForm} className="space-y-6">
          <div className="flex bg-slate-100 p-1 rounded-2xl shadow-inner">
            {[
              { label: 'Masuk', icon: ArrowDownRight, activeColor: 'bg-emerald-500', hoverColor: 'hover:bg-emerald-50', textActive: 'text-white', textInactive: 'text-slate-500' },
              { label: 'Keluar', icon: ArrowUpRight, activeColor: 'bg-rose-500', hoverColor: 'hover:bg-rose-50', textActive: 'text-white', textInactive: 'text-slate-500' }
            ].map(({ label, icon: Icon, activeColor, hoverColor, textActive, textInactive }) => (
              <button
                key={label}
                type="button"
                onClick={() => handleMutasiChange(label)}
                className={`
                  relative flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-black text-sm transition-all duration-300
                  ${formData.mutasi === label 
                    ? `${activeColor} ${textActive} shadow-md transform scale-[0.98]` 
                    : `${textInactive} ${hoverColor} hover:scale-105`
                  }
                `}
              >
                <Icon size={16} strokeWidth={2.5} className={formData.mutasi === label ? 'animate-pulse' : ''} />
                UANG {label.toUpperCase()}
                {formData.mutasi === label && (
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                {/* Tentukan opsi berdasarkan mutasi */}
                {(() => {
                  const currentJenisOptions = formData.mutasi === 'Masuk' ? jenisOptionsIn : jenisOptionsOut;
                  return (
                    <select name="jenis" value={formData.jenis || ''} onChange={handleInputChange} required className="w-full mt-1.5 p-3.5 bg-slate-50 border-none rounded-2xl outline-none focus:ring-2 focus:ring-emerald-500/20 font-bold text-slate-600">
                      ...
                      {currentJenisOptions.map(opt => <option key={opt.id} value={opt.text_1}>{opt.text_1}</option>)}
                      {currentJenisOptions.length === 0 && <option disabled>Data Jenis Tidak Ditemukan untuk mutasi ini</option>}
                    </select>
                  );
                })()}
              </div>
            ...
            {/* Field Akun Tujuan (hanya untuk transfer) */}
            {formData.jenis?.toLowerCase() === 'transfer' && (
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Akun Tujuan</label>
                <select
                  name="account_2"
                  value={formData.account_2 || ''}
                  onChange={handleInputChange}
                  required
                  className="w-full mt-1.5 p-3.5 bg-slate-50 border-none rounded-2xl outline-none focus:ring-2 focus:ring-emerald-500/20 font-bold text-slate-600"
                >
            ...
            <div className="pt-2">
              ...
              <input 
                ...
                onChange={(e) => {
                  ...
                  if (valid.length !== selected.length) alert("Beberapa file dilewati karena lebih dari 5MB atau format tidak valid.");
                  setFiles(prevFiles => {
                    const newFilesList = [...prevFiles]; 
                    valid.forEach(newFile => {
                      const isDuplicate = newFilesList.some(existingFile => existingFile.name === newFile.name && existingFile.size === newFile.size);
                      if (!isDuplicate) newFilesList.push(newFile); 
                    });
                    return newFilesList;
                  });
                  e.target.value = '';
          ...
          <div className="flex gap-3 pt-4">
            <button type="button" onClick={handleCloseModal} className="flex-1 py-4 bg-slate-100 rounded-2xl font-black text-slate-400 hover:bg-slate-200 transition-colors">BATAL</button>
      ...
      {galleryIndex !== null && selectedTx && selectedTx.file && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-sm flex items-center justify-center animate-fadeIn" onClick={() => setGalleryIndex(null)}>
          <button onClick={() => setGalleryIndex(null)} className="absolute top-6 right-6 md:top-10 md:right-10 p-3 text-white/70 hover:text-white bg-white/10 hover:bg-rose-500 rounded-full transition-all z-10">
            <X size={24} />
          </button>
          <button onClick={(e) => { e.stopPropagation(); setGalleryIndex(prev => prev! > 0 ? prev! - 1 : selectedTx.file.length - 1); }} className="absolute left-4 md:left-10 p-4 text-white/70 hover:text-white bg-white/10 hover:bg-white/30 rounded-full transition-all z-10">
            <ChevronLeft size={32} />
          </button>
          <div className="max-w-[90vw] max-h-[85vh] flex items-center justify-center p-4 relative group" onClick={(e) => e.stopPropagation()}>
            {isVideo(selectedTx.file[galleryIndex]) ? (
              <video src={pb.files.getUrl(selectedTx, selectedTx.file[galleryIndex])} controls autoPlay className="max-w-full max-h-full rounded-2xl shadow-2xl ring-1 ring-white/10" />
            ) : (
              <img src={pb.files.getUrl(selectedTx, selectedTx.file[galleryIndex])} alt="Gallery view" className="max-w-full max-h-[85vh] object-contain rounded-2xl shadow-2xl ring-1 ring-white/10" />
            )}
          </div>
          <button onClick={(e) => { e.stopPropagation(); setGalleryIndex(prev => prev! < selectedTx.file.length - 1 ? prev! + 1 : 0); }} className="absolute right-4 md:right-10 p-4 text-white/70 hover:text-white bg-white/10 hover:bg-white/30 rounded-full transition-all z-10">
          ...
          {selectedTx.file.length > 1 && (
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-3" onClick={(e) => e.stopPropagation()}>
              {selectedTx.file.map((_, idx) => (
                <span key={idx} className={`h-2 rounded-full transition-all duration-300 ${idx === galleryIndex ? 'w-8 bg-emerald-500' : 'w-2 bg-white/30'}`} />
              ))}
            </div>
          )}
        </div>
